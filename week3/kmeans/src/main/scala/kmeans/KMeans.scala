...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val genMap = points.par.groupBy(findClosest(_, means))
    means.par.map(x => x -> genMap.getOrElse(x, Nil)).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean =
      oldMeans.par forall(x => x.squareDistance(newMeans(oldMeans.indexOf(x))) <= eta)
