...
class BlurSuite extends FunSuite {
  ...
  test("blur image of 32 x 64") {
    val src = new Img(32, 64)
    val des = new Img(32, 64)

    for(x <- 0 until 32; y <- 0 until 64)
      src(x, y) = rgba(x, y, x + y, math.abs(x - y))

    VerticalBoxBlur.parBlur(src, des, 2, 2)

    VerticalBoxBlur.parBlur(src, des, 5, 3)

    VerticalBoxBlur.parBlur(src, des, 33, 3)
  }
