...
object ParallelCountChange {
  ...
  def countChange(money: Int, coins: List[Int]): Int =
    (money, coins) match {
      case (money, _) if money <= 0 => if(money < 0) 0 else 1
      case(money, c :: Nil) if money <= c => if(money < c) 0 else 1
      case(_, Nil) => 0
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int =
    ...
    else parallel(parCountChange(money - coins.head, coins, threshold),
      parCountChange(money, coins.tail, threshold)) match {
      case (x, y) => x + y
    }
